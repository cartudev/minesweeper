...
function explodemines(number){
    ...
    `<div style="background-color:${Colors[bg1][1]}"></div>
    <div></div>`)
    for(let i=1; i<= 4;i++){
    bg2 = randomIntFromInterval(0,1);
    bg3 = randomIntFromInterval(0,1);
    
    positionRep.insertAdjacentHTML('beforeend', 
    `<div style="--exp_background1: ${Colors[bg1][bg2]};
    --exp_background2: ${Colors[bg1][bg3]};
    --animation1: bookmark-anim${randomIntFromInterval(1,20)};
    --animation2: bookmark-anim${randomIntFromInterval(1,20)};
    --time1: ${randomFloatInterval(4.5,6.5,1)}s;
    --time2: ${randomFloatInterval(4.5,6.5,1)}s;
    ...
    for (let explode of minesPositions){
        let myTimeout = setTimeout(animationExp, randomIntFromInterval(1000,75000), explode);
